...
class FirebaseConnectionController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $this->database->getReference('quiz')->set([]);//set node kosong on databse its a table

            $field = $this->database->getReference('quiz/id')->set(''); //set key node or on databse its a column
            $field = $this->database->getReference('users/name')->set(''); //set key node or on databse its a column
